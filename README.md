...
<div>
  это последовательность шагов, которые выполняет браузер, когда преобразуется HTML, CSS и JavaScript в пиксели, которые вы видите на экране. Оптимизация этих шагов улучшает производительность рендера.
</div>
</details>
...
<div>
  Например, это может быть строка или число, или булевое значение.
</div>
</details>
...
heap-это память, выделенная для динамического выделения.

Стек - это память, выделенная в качестве свободного места для потока выполнения. 
</div>
</details>
...
<div>
  До пришествия E6 объявления через var(переменная которая могла быть глобальной и локальной),

  в отличие от var, let-переменные нельзя повторно объявить внутри области видимости

  const, сохраняют постоянные значения.
</div>
</details>
...
<div>
 ...
 в стандарте ES2015 было введено классовый синтаксис, то есть появилось ключевое слово class: которое является синтаксическим сахаром над прототипным наследованием.
</div>
</details>
...
<div>
  ип объединения Union — это эффективный способ выразить значение, которое может быть одного из нескольких типов. Два или больше типов данных можно комбинировать при помощи символа конвейеризации (|), обозначающего тип Union.
...
<div>
  Он известен, под названием встроенный шаблон. Существуют и другие способы того, как определить в Angular 2 шаблоны, которые могут быть выполнены с помощью команды templateURL
</div>
</details>
...
<div>
  Реальный DOM будет проводить обновление всех тегов, пока не найдёт нужный фрагмент. А это в некоторых случаях негативно сказывается на производительности и остальных параметрах. Виртуальный DOM обновит только необходимый фрагмент HTML.
</div>
</details>
...
<div>
   Stream может быть преобразован в Observable и везде, где ожидается Stream, можно присваивать Observable. Так что не путайтесь, если я буду смешивать эти термины в процессе объяснения
</div>
</details>
